function dy = NChainedAtomsAlex47(t,y,eta,D,w,sigma,hl,hr,H,k,l,Nafix,Nafree)
...
for i = 1:Nafree
    for j = 0:2*Nafix
        ...
        Fwall(i,:) = Fwall(i,:) + fl + fr; % so it stores all of the left forces for free atom i in one spot 
    end
end
...
for i = 1:(Nafree - 1)
    Rspring = (-1)*[y(2*i-1),y(2*i)]+[y(2*i+1),y(2*i+2)]; % 'spring' vector from atom above
    Fspring(i,:) = k*(norm(Rspring)-l)*(Rspring)/norm(Rspring);
end
...
% sum up the forces from the fixed atoms (the walls) and throw them in to the eqn
for i = 1:Nafree    
        dy(2*Nafree+(2*i-1)) = dy(2*Nafree+(2*i-1)) + Fwall(i,1);
        dy(2*Nafree+(2*i)) = dy(2*Nafree+(2*i)) + Fwall(i,2);
end

% now we must also use their forces on each other:
for i = 1:Nafree
        dy(2*Nafree+(2*i-1)) = dy(2*Nafree+(2*i-1)) + Fspring(i,1);
        dy(2*Nafree+(2*i)) = dy(2*Nafree+(2*i)) + Fspring(i,2);
end
