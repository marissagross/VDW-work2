function dy = NChainedAtomsManny(t,y,eta,D,w,sigma,hl,hr,H,k,l,Nafix,Nafree)
...
for i = 1:Nafree
        dy(2*Nafree+(2*i-1)) = dy(2*Nafree+(2*i-1)) + F(i,1) + Fspring(i,1) - eta*y(2*Nafree+(2*i-1);
        dy(2*Nafree+(2*i)) = dy(2*Nafree+(2*i)) + F(i,2) + Fspring(i,2) - eta*y(2*Nafree+(2*i));
end
